...
interface ShowRepository {
    // Basic operations
    suspend fun getShows(next: String? = null, limit: Int = 20, filter: ShowFilter? = null): Result<List<Show>>
    suspend fun getShows(ids: List<String>, relationships: List<String> = emptyList()): Result<List<Show>>
    suspend fun getShow(id: String, relationships: List<String> = emptyList()): Result<Show>
    suspend fun getUpcomingShows(next: String? = null, limit: Int = 20): Result<List<Show>>
    // Related content
    ...
    // Rating
    suspend fun rateShow(showId: String, rating: Double, review: String? = null): Result<Show>
}
...
open class ShowRepositoryImpl(
    ...
    override suspend fun getShows(next: String?, limit: Int, filter: ShowFilter?): Result<List<Show>> {
    ...
    override suspend fun getShows(ids: List<String>, relationships: List<String>): Result<List<Show>> {
        val parameters: MutableMap<String, String> = mutableMapOf()
        if (!relationships.isEmpty()) {
            parameters.put("include", relationships.joinToString(","))
        }
        return apiClient.get<ShowResponse>(KKEndpoint.Show.Index(ids), parameters).map { it.data }
    }
    ...
    override suspend fun getUpcomingShows(next: String?, limit: Int): Result<List<Show>> {
    ...
    override suspend fun getShowCast(showId: String, next: String?, limit: Int): Result<List<Person>> {
        val parameters = mapOf("limit" to limit.toString())
        return apiClient.get<PersonResponse>(KKEndpoint.Show.Cast(showId), parameters).map { it.data }
    }
    ...
    override suspend fun getShowSeasons(showId: String, next: String?, limit: Int): Result<List<Season>> {
        ...
        return apiClient.get<SeasonResponse>(endpoint, parameters).map { it.data }
    }
    ...
    override suspend fun rateShow(showId: String, rating: Double, review: String?): Result<Show> {
        val body = mapOf(
            "rating" to rating,
            "review" to review
        ).filterValues { it != null }

        return apiClient.post<ShowResponse, Map<String, Any?>>(KKEndpoint.Show.Rate(showId), body).map { it.data.first() }
    }
}
