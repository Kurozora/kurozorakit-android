...
interface UserRepository {
    ...
    suspend fun getMyFollowers(next: String? = null, limit: Int = 20): Result<List<User>>
    suspend fun getMyFollowing(next: String? = null, limit: Int = 20): Result<List<User>>
    // Access tokens
    ...
    suspend fun updateAccessToken(accessToken: String, apnDeviceToken: String): Result<AccessToken>
    ...
    // Library
    suspend fun getMyLibrary(libraryKind: KKLibrary.Kind, libraryStatus: KKLibrary.Status, sortType: KKLibrary.SortType, sortOption: KKLibrary.Option, next: String? = null, limit: Int = 20): Result<Map<String, Any>>
    suspend fun addToLibrary(libraryKind: KKLibrary.Kind, libraryStatus: KKLibrary.Status, modelID: String): Result<Unit>
    suspend fun updateMyLibrary(libraryKind: KKLibrary.Kind, modelID: String, rewatchCount: Int? = null, isHidden: Boolean? = null): Result<Map<String, Any>>
    suspend fun removeFromMyLibrary(libraryKind: KKLibrary.Kind, modelID: String): Result<Unit>
    ...
    // Sessions
    suspend fun getMySessions(next: String? = null, limit: Int = 20): Result<List<Session>>
...
open class UserRepositoryImpl(
    ...
    override suspend fun getMyFollowers(next: String?, limit: Int): Result<List<User>> {
    ...
    override suspend fun getMyFollowing(next: String?, limit: Int): Result<List<User>> {
        val parameters = mapOf("limit" to limit.toString())
    ...
    override suspend fun updateAccessToken(accessToken: String, apnDeviceToken: String): Result<AccessToken> {
        val body = mapOf("apn_device_token" to apnDeviceToken)
        val tokenID = accessToken.split('|')[0]
        return apiClient.put<AccessTokenResponse, Map<String, String>>(KKEndpoint.Me.AccessTokens.Update(tokenID), body).map { it.data.first() }
    }
    ...
    override suspend fun getMyFeedMessages(next: String?, limit: Int): Result<List<FeedMessage>> {
        ...
        return apiClient.get<FeedMessageResponse>(KKEndpoint.Me.Feed.Messages, parameters).map { it.data }
    }

    override suspend fun getMyLibrary(libraryKind: KKLibrary.Kind, libraryStatus: KKLibrary.Status, sortType: KKLibrary.SortType, sortOption: KKLibrary.Option, next: String?, limit: Int): Result<Map<String, Any>> {
        ...
        return apiClient.get<Map<String, Any>>(endpoint, parameters)
    }

    override suspend fun addToLibrary(libraryKind: KKLibrary.Kind, libraryStatus: KKLibrary.Status, modelID: String): Result<Unit> {
        val body = mapOf(
            "library" to libraryKind.stringValue,
            "status" to libraryStatus.sectionValue,
            "model_id" to modelID
        )
        return apiClient.post<Unit, Map<String, String>>(KKEndpoint.Me.Library.Index, body)
    }

    override suspend fun updateMyLibrary(libraryKind: KKLibrary.Kind, modelID: String, rewatchCount: Int?, isHidden: Boolean?): Result<Map<String, Any>> {
        val body = mapOf(
            "library" to libraryKind.stringValue,
            "model_id" to modelID,
        ...
        return apiClient.put<Map<String, Any>, Map<String, Any?>>(KKEndpoint.Me.Library.Update, body)
    }

    override suspend fun removeFromMyLibrary(libraryKind: KKLibrary.Kind, modelID: String): Result<Unit> {
        val body = mapOf(
            "model_id" to modelID,
            "library" to libraryKind.stringValue
        )
        return apiClient.post<Unit, Map<String, String>>(KKEndpoint.Me.Library.Delete, body)
    }
    ...
    override suspend fun getMySessions(next: String?, limit: Int): Result<List<Session>> {
        val parameters = mapOf("limit" to limit.toString())
        val endpoint: KKEndpoint = next?.let { KKEndpoint.Url(it) } ?: KKEndpoint.Me.Sessions.Index
        return apiClient.get<SessionResponse>(endpoint, parameters).map { it.data }
    }
